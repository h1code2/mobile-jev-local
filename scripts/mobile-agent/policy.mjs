  ...
  async decide({ goal, observation, history = [], texts = [], apps = [] }) {
    ...
    const body = {
      ...
      state: {
        ...
        ...(observation.facts
          ? {
              deviceFacts: observation.facts,
              rules:
                'deviceFacts are ground truth reported by the device itself. When the goal asks about a value that deviceFacts already answers (version, build, model), choose DONE with that evidence; do not require the same value to also appear on screen.',
            }
          : {}),
